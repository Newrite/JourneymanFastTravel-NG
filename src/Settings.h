...
class Settings
{
  ...
  void LoadSettings()
  {
    ...
    RequiredItems =
        RE::TESDataHandler::GetSingleton()->LookupForm(RE::FormID(0x800), "Journeyman.esp")->As<RE::BGSListForm>();
    MAG_IsFeatureLocked =
        RE::TESDataHandler::GetSingleton()->LookupForm(RE::FormID(0x80A), "Journeyman.esp")->As<RE::TESGlobal>();
    UpdateFeatureLocked();
    Survival_ModeEnabledShared = dataHandler->LookupForm(RE::FormID(0x314A), "Update.esm")->As<RE::TESGlobal>();
    logger::info("Settings loaded");
  }

  void UpdateFeatureLocked() { MAG_IsFeatureLocked->value = EnableOnlyOnSM; }
